...
class BacktestEngine:
    ...
    def load_data(self, ticker: str) -> pd.DataFrame:
        ...
        """
        return self.market_data_agent.fetch_data(ticker)
    ...
    def _generate_results(
        ...
        data['benchmark_equity'] = (1 + data['returns']).cumprod() * 100000  # Starting with $100k
        ...
        results.columns = ['strategy', 'benchmark']
    ...
    def plot_results(self, save_path: str = None):
        ...
        ax1.plot(equity['date'], equity['strategy'], label='Strategy', linewidth=2)
        ax1.plot(equity['date'], equity['benchmark'], label='Buy & Hold', linestyle='--', alpha=0.7)
        ...
        # Plot drawdown
        rolling_max = equity['strategy'].cummax()
        drawdown = (equity['strategy'] - rolling_max) / rolling_max * 100
    ...
    def print_summary(self):
        ...
        print("\n" + "="*50)
        print(f"BACKTEST SUMMARY - {res['ticker']}")
        print(f"Period: {res['start_date'].strftime('%Y-%m-%d')} to {res['end_date'].strftime('%Y-%m-%d')}")
        print("-"*50)
